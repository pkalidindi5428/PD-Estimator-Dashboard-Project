  ...
  if($filter) {
      switch ($_GET['filter']) {
          case 1:
              for ($i = 2; $i <= count($sheetData); $i++) {
                  ...
                  if (strcmp("$phase", "$pjt") != "0") {
                      ...
                      $plan_phase[$a][0][11] = 1; //init - filter count
                      //$c++;
          ...
          case 2:
              for ($i = 2; $i <= count($sheetData); $i++) {
                  $path = explode("\\", $sheetData[$i][A]);
                  $server_type_num = count($path, 0);
                  $server_type_filter = $path[$server_type_num - 1];
                  if($path[8] != "Server Platform Specific"){
                      $server_type_filter = "Not Server Platform Specific";
                  }
                  $test_id = $sheetData[$i][D];
                  $PD = LookupTestPD($test_id);
                  ...
                  if (strcmp("$phase", "$pjt") != "0") {
                      ...
                      $plan_phase[$a][$c][8] = 0; //init - Blocked PD
                      $plan_phase[$a][$c][9] = 0; //init - No run + Inprogress PD
                      $plan_phase[$a][$c][10] = $server_type_filter; //init - filter
                      $plan_phase[$a][0][11] = 1; //init - filter count
                      //$c++;
                  } elseif (strcmp("$feature", "$path[6]") != "0") {
                      //same phase, another feature
                      ...
                      for ($x = 0; $x < $c; $x++) {
                          if($server_type_filter == $plan_phase[$a][$x][10]){
                              $plan_phase[$a][$x][1] = $plan_phase[$a][$x][1] + $PD;
                              //print("same phase/feature -->".$feature."<br>");
                              $found = 1;
                              $c = $x;
          ...
          case 3:
              for ($i = 2; $i <= count($sheetData); $i++) {
                  $path = explode("\\", $sheetData[$i][A]);
                  $test_set_name = explode("|", $sheetData[$i][B]);
                  $test_set_num = count($test_set_name, 0);
                  $test_set_filter = $test_set_name[$test_set_num - 1];
                  if($test_set_filter == ""){
                      $test_set_filter = $test_set_name[$test_set_num - 2];
                  }
                  $test_id = $sheetData[$i][D];
                  $PD = LookupTestPD($test_id);
                  ...
                  if (strcmp("$phase", "$pjt") != "0") {
                      ...
                      $plan_phase[$a][$c][8] = 0; //init - Blocked PD
                      $plan_phase[$a][$c][9] = 0; //init - No run + Inprogress PD
                      $plan_phase[$a][$c][10] = $test_set_filter; //init - filter
                      $plan_phase[$a][0][11] = 1; //init - filter count
                      //$c++;
                  } elseif (strcmp("$feature", "$path[6]") != "0") {
                      //same phase, another feature
                      ...
                      for ($x = 0; $x < $c; $x++) {
                          if($test_set_filter == $plan_phase[$a][$x][10]){
                              $plan_phase[$a][$x][1] = $plan_phase[$a][$x][1] + $PD;
                              //print("same phase/feature -->".$feature."<br>");
                              $found = 1;
                              $c = $x;
...
/*
        for($i = 0; $i < $total_phase; $i++){
            for($j = 0; $j < $plan_phase[$i][0][11]; $j++) {
                if ($plan_phase[$i][$j][10] == "Automation") {
                    $plan_copy[$i][0][0] = $plan_phase[$i][j][0]; //init - phase
                    $plan_copy[$i][0][1] = $plan_phase[$i][j][1];  //phase PD
                    $plan_copy[$i][0][2] += $plan_phase[$i][j][2];
                    $plan_copy[$i][0][3] += $plan_phase[$i][j][3];
                    $plan_copy[$i][0][4] += $plan_phase[$i][j][4];
                    $plan_copy[$i][0][5] += $plan_phase[$i][j][5];
                    $plan_copy[$i][0][6] += $plan_phase[$i][j][6];
                    $plan_copy[$i][0][7] += $plan_phase[$i][j][7];
                    $plan_copy[$i][0][8] += $plan_phase[$i][j][8];
                    $plan_copy[$i][0][9] += $plan_phase[$i][j][9];
                    $plan_copy[$i][0][10] = $plan_phase[$i][j][10]; //init - filter
                    }
                if ($plan_phase[$i][$j][10] == "Manual") {
                    $plan_copy[$i][1][0] = $plan_phase[$i][j][0]; //init - phase
                    $plan_copy[$i][1][1] = $plan_phase[$i][j][1];  //phase PD
                    $plan_copy[$i][1][2] += $plan_phase[$i][j][2];
                    $plan_copy[$i][1][3] += $plan_phase[$i][j][3];
                    $plan_copy[$i][1][4] += $plan_phase[$i][j][4];
                    $plan_copy[$i][1][5] += $plan_phase[$i][j][5];
                    $plan_copy[$i][1][6] += $plan_phase[$i][j][6];
                    $plan_copy[$i][1][7] += $plan_phase[$i][j][7];
                    $plan_copy[$i][1][8] += $plan_phase[$i][j][8];
                    $plan_copy[$i][1][9] += $plan_phase[$i][j][9];
                    $plan_copy[$i][1][10] = $plan_phase[$i][j][10]; //init - filter
                }
            }
        }
*/
...
<div id="qaContent" style=" float:left;">
    ...
    switch ($_GET['filter']) {
        case 1:
            ...
            for ($i = 0; $i < $total_phase; $i++) {
                for ($j = 0; $j < $plan_phase[$i][0][11]; $j++) {
                    $tc_total[$i][0] += $plan_phase[$i][$j][2] + $plan_phase[$i][$j][3] + $plan_phase[$i][$j][4] + $plan_phase[$i][$j][5] + $plan_phase[$i][$j][6] + $plan_phase[$i][$j][7];
                    $tc_total[$i][1] += $plan_phase[$i][$j][1];
                    $tc_total[$i][2] += $plan_phase[$i][$j][5] + $plan_phase[$i][$j][6];
                    $tc_total[$i][3] += $plan_phase[$i][$j][9];
                    $tc_total[$i][4] += $plan_phase[$i][$j][2];
                    $tc_total[$i][5] += $plan_phase[$i][$j][8];
                }
            }
            ...
            for ($i = 0; $i < $total_phase; $i++) {
                ...
                print($plan_phase[$i][0][0] . " ( ");
                echo '<font color="#000000">';
                // $tc_planned = $plan_phase[$i][2] + $plan_phase[$i][3] + $plan_phase[$i][4] + $plan_phase[$i][5] + $plan_phase[$i][6] + $plan_phase[$i][7];
                print("Total = " . $tc_total[$i][0] . " [" . $tc_total[$i][1] . "]");  //total
                echo '</font>';
                echo ' / ';
                echo '<font color="#AC58FA">';
                // $tc_norun = $plan_phase[$i][5] + $plan_phase[$i][6];
                print("No Run = " . $tc_total[$i][2] . " [" . $tc_total[$i][3] . "]"); //No run + in progress
                echo '</font>';
                echo ' / ';
                echo '<font color="#FF8000">';
                print(" Blocked = " . $tc_total[$i][4] . " [" . $tc_total[$i][5] . "]"); //blocked
                echo '</font>';
                echo ' )';
                echo '</div>';
